...
class Cascade_Detector:
    ...
    # Class Variables
    # Resolve the cascade file relative to the installed Python package, not
    # the current working directory.  `ros2 run` may be launched from any
    # directory, so os.getcwd() is not reliable here.
    TrafficLight_cascade_str = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", "..", "data", "TrafficLight_cascade.xml")
    )
    TrafficLight_cascade = cv2.CascadeClassifier()
    #-- 1. Load the cascades
    if not os.path.exists(TrafficLight_cascade_str) or not TrafficLight_cascade.load(TrafficLight_cascade_str):
        raise FileNotFoundError(f"Could not load traffic light cascade: {TrafficLight_cascade_str}")
