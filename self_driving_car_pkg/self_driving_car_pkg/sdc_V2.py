...
# Compatibility shim: this project was written for the OpenCV 3 findContours
# API: image, contours, hierarchy = cv2.findContours(...), so many modules use
# cv2.findContours(...)[1]. OpenCV 4 returns only contours, hierarchy; without
# this shim those call sites receive hierarchy/None and crash.
if not hasattr(cv2, "_sdc_original_findContours"):
    cv2._sdc_original_findContours = cv2.findContours

    def _sdc_findContours_compat(*args, **kwargs):
        result = cv2._sdc_original_findContours(*args, **kwargs)
        if len(result) == 2:
            contours, hierarchy = result
            return None, contours, hierarchy
        return result

    cv2.findContours = _sdc_findContours_compat
