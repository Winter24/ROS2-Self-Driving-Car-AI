...
def disp_on_mydev(screen,device="tablet"):
    # Resolve resources independent of the current working directory.
    candidates = [
        os.path.join(os.path.dirname(__file__), "resource", device + ".png"),
    ]
    try:
        candidates.append(os.path.join(
            get_package_share_directory("self_driving_car_pkg"),
            "self_driving_car_pkg", "GPS_Navigation", "resource", device + ".png"
        ))
    except Exception:
        pass

    device_path = next((path for path in candidates if os.path.exists(path)), candidates[0])
    device_view = cv2.imread(device_path)
    if device_view is None:
        raise FileNotFoundError(f"Could not read GPS_Navigation device image: {device_path}")
